...
// Integration test for direct reload behavior on an unchanged file.
// Verifies that first direct reload in a session returns 202 Accepted and
// subsequent direct reload returns 304 Not Modified.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn server_direct_reload_ok_then_not_modified() {
    // Choose a free port
    let port = {
        let sock = std::net::TcpListener::bind(SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 0))
            .expect("bind 0");
    ...
    examples.push("..");
    examples.push("examples");
    let src = examples.join("counter.vcd");
    ...
    // Copy initial file to destination
    fs::copy(&src, &dest).expect("copy counter.vcd to temp");
    ...
    // 1) First direct reload for unchanged file -> 200 OK
    let resp = client.get(format!("{base}/0/reload")).send().await.unwrap();
    assert_eq!(
        resp.status(),
        StatusCode::ACCEPTED,
        "first direct unchanged reload should yield 202 Accepted"
    );

    // Wait until the direct reload finishes before checking unchanged behavior.
    let mut reload_complete = false;
    for _ in 0..100 {
        // up to ~10s
        let st_resp = client
    ...
    assert!(
        reload_complete,
        "first direct reload did not finish in expected time"
    );

    // 2) Subsequent unchanged reload -> 304 Not Modified
    let resp = client.get(format!("{base}/0/reload")).send().await.unwrap();
    assert_eq!(
        resp.status(),
        StatusCode::NOT_MODIFIED,
        "subsequent unchanged reload should yield 304"
    );
